def get_cover_html(img_w, img_h):
    ...
    img_htmls.append(
        '    <svg xmlns="http://www.w3.org/2000/svg" height="100%" preserveAspectRatio="xMidYMid meet" version="1.1" viewBox="0 0 '
        + str(img_w)
        + " "
        + str(img_h)
...
def get_toc_html(title, chap_names, img_exist=False, image_xhtml=False):
    toc_htmls = []
    ...
    if img_exist:
        ...
        toc_htmls.append("        <text>彩页</text>\n")
        toc_htmls.append("      </navLabel>\n")
        toc_htmls.append('      <content src="Text/color.xhtml"/>\n')
    ...
    for chap_no, chap_name in enumerate(chap_names):
        ...
        toc_htmls.append("      </navLabel>\n")
        toc_htmls.append(
            '      <content src="Text/' + str(chap_no).zfill(2) + '.xhtml"/>\n'
        )
        toc_htmls.append("    </navPoint>\n")
        current_order += 1
    ...
    if image_xhtml:
        ...
        toc_htmls.append("      <navLabel>\n")
        toc_htmls.append("        <text>插图</text>\n")
        toc_htmls.append("      </navLabel>\n")
        toc_htmls.append('      <content src="Text/image.xhtml"/>\n')
        toc_htmls.append("    </navPoint>\n")
        current_order += 1
...
def get_content_html(
    title, author, num_chap, num_img, img_exist=False, image_xhtml=False
):
    content_htmls = []
    ...
    content_htmls.append(
        '    <dc:identifier id="BookId" opf:scheme="UUID">urn:uuid:942b8224-476b-463b-9078-cdfab0ee2686</dc:identifier>\n'
    )
    content_htmls.append("    <dc:language>zh</dc:language>\n")
    content_htmls.append("    <dc:title>" + title + "</dc:title>\n")
    content_htmls.append(
        '    <dc:creator opf:role="aut" opf:file-as="未知">'
        + author
        + "</dc:creator>\n"
    )
    ...
    if image_xhtml:
        content_htmls.append(
            '    <item id="ximage" href="Text/image.xhtml" media-type="application/xhtml+xml"/>\n'
        )
    ...
    for chap_no in range(num_chap):
        content_htmls.append(
            '    <itemref idref="x' + str(chap_no).zfill(2) + '.xhtml"/>\n'
        )

    if image_xhtml:
        content_htmls.append('    <itemref idref="ximage"/>\n')
