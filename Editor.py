...
class Editor(object):
    def __init__(self, root_path, book_no="0000", volume_no=1):
        ...
        if len(cover_list) > 1:
            self.cover_url = cover_list[1]
        elif len(cover_list) > 0:
            self.cover_url = cover_list[0]
        else:
            self.cover_url = ""
        ...
        # 5. Clean the title for filesystem safety
            ...
            if line.startswith('<td class="vcss"'):
                ...
                if chap_name != "" and chap_name != " ":
                    ...
                    chap_urls_list[-1].append(
                        self.cata_page.replace("index.htm", bf.find("a")["href"])
                    )
        if is_print:
    ...
    def get_chap_text(self, url, chap_name, is_color=False):
        ...
        if is_color:
            ...
            for line in text_html.split("\n"):
                if not line.startswith('<ul id="contentdp">') and not line.startswith(
                    "<br/>"
                ):
                    text_chap += line + "\n"
    ...
    def check_volume(self, is_gui=False, signal=None, editline=None):
        ...
        if self.color_chap_name not in self.volume["chap_names"]:
            ...
            print("**************")
            print(
                "提示：没有彩页，但主页封面存在，将使用主页的封面图片作为本卷图书封面"
            )
            print("**************")
